...
class SPARQLQueryDispatcher {
  ...
  query(sparqlQuery) {
    const fullUrl = this.endpoint + "?query=" + encodeURIComponent(sparqlQuery); //concatenates the endpoint property with the input query to form the full URL used to send an HTTP request
...
/*________________________Fetching the Medications - Initiated by the fetchMeds() function_________________________________________________________________________________________________________________*/
                                                       //fetchMeds() - medications.js Ln 22
async function medicationQuery(input) {
  const template =//defines a template query, we intend to replace the "DISEASE" with the variable for which we actually want to run the query
...
/*________________________Fetching the Interactions - Initiated by the fetchInteractions() function_________________________________________________________________________________________________________________*/
                                                       //fetchInteractions() - Interactions.js Ln 2
async function interactionsQuery(diseaseA, filter) {
  const template = `SELECT DISTINCT ?medicine ?medicineLabel (group_concat(?interactswith;separator=",") as ?interactions) (group_concat(?interactswithLabel;separator=",") as ?interactionsLabel)
  WHERE {
    SERVICE wikibase:label { bd:serviceParam wikibase:language "[AUTO_LANGUAGE], en". 
...
GROUP BY ?medicine ?medicineLabel`;
  const query = template
    .replaceAll("DISEASEA", diseaseA)           // replaces the string "DISEASEA" in the template by the input disease
    .replace("INTERACTIONS", filter);           // replaces the string "INTERACTIONS" in the template with the filter variable defined 
  try {                                         // in and passed on by the fetchInteractions() Function
    const result = await runQuery(query);       // if the query is succesfull the following will run
    return result;                              // returns the result to the function it was called by 
  } catch (error) {
    alert(error);                               // if the query can not be succesfully finished it gives an error in the browser.
  }
}

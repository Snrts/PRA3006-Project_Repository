async function loadDropdown() {
  ...
  async function fetchMeds(selection) {
    ...
    for (var j = 0, m = outputMedicationQuery.length; j < m; j++) {
      medicationList.push([
        outputMedicationQuery[j].medicine.value.slice(31),
        outputMedicationQuery[j].medicineLabel.value,
      ]); //adds the medication to the list, once again the slice makes sure we only get the item code

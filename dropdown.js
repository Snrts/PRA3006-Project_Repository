...
//________________________________________________________________________________________________//
    $("#submit").on("click", function () { //adds event listener in the button in the html file with the id "submit"
        ...
        fetchMeds(options) // passes on the selected diseases and initiates the query to fetch their medicines
    })
...
{   
    ...
    for (var i = 0, l = selection.length; i < l; i++)
    {
        ...
        {
            var interactions = await fetchInteractions(list[j].medicine.value.slice(31)) 
            const pls = []
            for (var a = 0, b = interactions.length; a < b; a++){
                pls.push(interactions[a].interactswith.value.slice(31))
            }
            medlist.push({
                diseasecode: selection[i].diseasecode,
                disease: selection[i].disease,
                medcode: list[j].medicine.value.slice(31),
                medname: list[j].medicineLabel.value,
                interactsw: pls
            })
